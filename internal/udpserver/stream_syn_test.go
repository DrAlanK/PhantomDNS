...
func TestHandleStreamSynFastPathsAlreadyConnectedStream(t *testing.T) {
	...
	stream := record.getOrCreateStream(7, s.streamARQConfig(record.ID, record.DownloadCompression), nil, s.log)
	if stream == nil {
...
func TestHandleSOCKS5SynFastPathsAlreadyConnectedStream(t *testing.T) {
	...
	stream := record.getOrCreateStream(6, s.streamARQConfig(record.ID, record.DownloadCompression), nil, s.log)
	if stream == nil {
		t.Fatal("expected stream to exist")
	}
	stream.mu.Lock()
...
func TestHandleSOCKS5SynImmediateRejectsBlockedTarget(t *testing.T) {
	...
	stream := record.getOrCreateStream(8, s.streamARQConfig(record.ID, record.DownloadCompression), nil, s.log)
	if stream == nil {
		t.Fatal("expected stream to exist")
	}
