...
## | 🇮🇷 [فارسی](https://github.com/DrAlanK/phantomdns/blob/main/README_FA.MD) | 🇬🇧 [English](https://github.com/DrAlanK/phantomdns/blob/main/README.MD) | 🇷🇺 [Русский](https://github.com/DrAlanK/phantomdns/blob/main/README_RU.MD) | 🇨🇳 [中文](https://github.com/DrAlanK/phantomdns/blob/main/README_ZH.MD) | 🇪🇸 [Español](https://github.com/DrAlanK/phantomdns/blob/main/README_ES.MD) | 🇮🇹 [Italiano](https://github.com/DrAlanK/phantomdns/blob/main/README_IT.MD) |

**phantomdns** - это научно-исследовательский проект, предназначенный для передачи TCP-трафика через DNS-запросы и ответы. В общем смысле он схож с проектами вроде DNSTT или SlipStream, однако использует принципиально иную архитектуру и подход к реализации.
...
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/DrAlanK/phantomdns)
...
<a href="https://trendshift.io/repositories/23688" target="_blank"><img src="https://trendshift.io/api/badge/repositories/23688" alt="DrAlanK%2Fphantomdns | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>

### 📊 Сравнение phantomdns с аналогичными проектами

| Особенность | SlipStream | DNSTT | phantomdns |
| :--- | :--- | :--- | :--- |
...
phantomdns предоставляется исключительно в образовательных и исследовательских целях.
...
- **Условия лицензии:** Использование, копирование, распространение или модификация данного программного обеспечения регулируются условиями лицензии MIT, изложенными в файле [LICENSE](https://github.com/DrAlanK/phantomdns/blob/main/LICENSE) данного репозитория. Любое использование, не соответствующее этим условиям, запрещено.

---

## Канал новостей и поддержки 📢

Чтобы быть в курсе последних новостей, релизов и новостей о проектах, подпишитесь на наш канал в Telegram: [Telegram Channel](https://t.me/phantomdns)
...
Краткий обзор основных возможностей phantomdns:
...
phantomdns — это не просто теоретический проект. Он прошел испытание в реальных условиях и доказал свою эффективность в ситуациях, когда глобальный интернет полностью отключен.
...
Стандартные инструменты обхода блокировок бесполезны, когда нет международного интернета, к которому можно подключиться. Однако во время этого масштабного отключения phantomdns выделился как один из немногих спасательных кругов, которые действительно позволили пользователям оставаться на связи с глобальной сетью.

**Как он выжил при полном отключении?**
Вместо того чтобы действовать как стандартный VPN, phantomdns использует интеллектуальные технологии DNS-туннелирования, чтобы пробиться сквозь блокировку:
...
Именно эта комбинация позволила phantomdns поддерживать стабильное соединение, когда доступ к внешнему миру был полностью заблокирован.
...
Если вы хотите развернуть сервер phantomdns на Linux, проще всего воспользоваться скриптом автоматической установки. Выполните на Linux сервере следующую команду:

```bash
bash <(curl -Ls https://raw.githubusercontent.com/DrAlanK/phantomdns/main/server_linux_install.sh)
```
...
| :--- | :--- | :--- | :--- |
| Windows 🪟 | `AMD64` (64-bit) | Windows 10 and 11 | [Download Windows Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Windows_AMD64.zip) |
| Windows 🪟 | `x86` (32-bit) | Older 32-bit Windows systems | [Download Windows x86 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Windows_X86.zip) |
| Windows 🪟 | `ARM64` | Windows on ARM devices | [Download Windows ARM64 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Windows_ARM64.zip) |
| macOS 🍎 | `ARM64` | Apple Silicon Macs (M1 / M2 / M3) | [Download macOS Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_MacOS_ARM64.zip) |
| macOS 🍎 | `AMD64` | Intel Macs | [Download macOS Intel Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_MacOS_AMD64.zip) |
| Linux 🐧 | `AMD64` (64-bit) | Modern distributions (Ubuntu 22.04+, Debian 12+) | [Download Linux Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_AMD64.zip) |
| Linux 🐧 | `x86` (32-bit) | Older 32-bit Linux systems | [Download Linux x86 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_X86.zip) |
| Linux (Legacy) 🐧 | `AMD64` (64-bit) | Older distributions (Ubuntu 20.04, Debian 11) | [Download Linux Legacy Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux-Legacy_AMD64.zip) |
| Linux (Legacy) 🐧 | `ARM64` | Older ARM64 Linux systems that need broader compatibility | [Download Linux Legacy ARM64 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux-Legacy_ARM64.zip) |
| Linux (ARM) 🐧 | `ARM64` | ARM servers, Raspberry Pi, and similar boards | [Download Linux ARM Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARM64.zip) |
| Linux (ARM) 🐧 | `ARMv7` | 32-bit ARM boards and older embedded Linux devices | [Download Linux ARMv7 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARMV7.zip) |
| Linux (ARM) 🐧 | `ARMv6` | Older ARM boards and lightweight Linux devices | [Download Linux ARMv6 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARMV6.zip) |
| Linux (ARM) 🐧 | `ARMv5` | Very old ARM devices and embedded Linux systems | [Download Linux ARMv5 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARMV5.zip) |
| Linux 🐧 | `RISCV64` | RISC-V Linux boards and servers | [Download Linux RISCV64 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_RISCV64.zip) |
| Linux (MIPS) 🐧 | `MIPS` | Big-endian MIPS Linux and router platforms | [Download Linux MIPS Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPS.zip) |
| Linux (MIPS) 🐧 | `MIPSLE` | Little-endian MIPS Linux and router platforms | [Download Linux MIPSLE Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPSLE.zip) |
| Linux (MIPS) 🐧 | `MIPS64` | 64-bit big-endian MIPS Linux systems | [Download Linux MIPS64 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPS64.zip) |
| Linux (MIPS) 🐧 | `MIPS64LE` | 64-bit little-endian MIPS Linux systems | [Download Linux MIPS64LE Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPS64LE.zip) |
| Termux / Android 📱 | `ARM64` | Modern Android phones running Termux | [Download Termux ARM64 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Termux_ARM64.zip) |
| Termux / Android 📱 | `ARMv7` | Older Android phones running 32-bit Termux environments | [Download Termux ARMv7 Client ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Termux_ARMV7.zip) |
...
| Операционная система | Архитектура | Совместимость | Загрузка |
| :--- | :--- | :--- | :--- |
| Windows 🪟 | `AMD64` (64-bit) | Windows Server, Windows 10 and 11 | [Download Windows Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Windows_AMD64.zip) |
| Windows 🪟 | `x86` (32-bit) | Older 32-bit Windows systems | [Download Windows x86 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Windows_X86.zip) |
| Windows 🪟 | `ARM64` | Windows on ARM devices | [Download Windows ARM64 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Windows_ARM64.zip) |
| Linux 🐧 | `AMD64` (64-bit) | Ubuntu 22.04+, Debian 12+ servers | [Download Linux Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_AMD64.zip) |
| Linux 🐧 | `x86` (32-bit) | Older 32-bit Linux systems | [Download Linux x86 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_X86.zip) |
| Linux (Legacy) 🐧 | `AMD64` (64-bit) | Older servers (Ubuntu 20.04, Debian 11) | [Download Linux Legacy Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux-Legacy_AMD64.zip) |
| Linux (Legacy) 🐧 | `ARM64` | Older ARM64 Linux systems that need broader compatibility | [Download Linux Legacy ARM64 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux-Legacy_ARM64.zip) |
| Linux (ARM) 🐧 | `ARM64` | ARM servers | [Download Linux ARM Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARM64.zip) |
| Linux (ARM) 🐧 | `ARMv7` | 32-bit ARM servers and embedded Linux devices | [Download Linux ARMv7 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARMV7.zip) |
| Linux (ARM) 🐧 | `ARMv6` | Older ARM boards and lightweight Linux devices | [Download Linux ARMv6 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARMV6.zip) |
| Linux (ARM) 🐧 | `ARMv5` | Very old ARM devices and embedded Linux systems | [Download Linux ARMv5 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARMV5.zip) |
| Linux 🐧 | `RISCV64` | RISC-V Linux boards and servers | [Download Linux RISCV64 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_RISCV64.zip) |
| Linux (MIPS) 🐧 | `MIPS` | Big-endian MIPS Linux and router platforms | [Download Linux MIPS Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPS.zip) |
| Linux (MIPS) 🐧 | `MIPSLE` | Little-endian MIPS Linux and router platforms | [Download Linux MIPSLE Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPSLE.zip) |
| Linux (MIPS) 🐧 | `MIPS64` | 64-bit big-endian MIPS Linux systems | [Download Linux MIPS64 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPS64.zip) |
| Linux (MIPS) 🐧 | `MIPS64LE` | 64-bit little-endian MIPS Linux systems | [Download Linux MIPS64LE Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPS64LE.zip) |
| macOS 🍎 | `ARM64` | Apple Silicon Macs | [Download macOS Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_MacOS_ARM64.zip) |
| macOS 🍎 | `AMD64` | Intel Macs | [Download macOS Intel Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_MacOS_AMD64.zip) |
| Termux / Android 📱 | `ARM64` | Modern Android / Termux environments | [Download Termux ARM64 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Termux_ARM64.zip) |
| Termux / Android 📱 | `ARMv7` | Older Android / 32-bit Termux environments | [Download Termux ARMv7 Server ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Termux_ARMV7.zip) |

---

### Раздел 2.2: 📦 phantomdns Docker Image

---

#### Раздел 2.2.1: ⚠️ Обзор

Этот Docker образ запускает сервер phantomdns в контейнерной среде и поддерживает сборки для нескольких архитектур.
...
docker run -d \
  ...
  -p 53:53/tcp \
  -p 53:53/udp \
  ghcr.io/DrAlanK/phantomdns:latest
```
...
services:
  phantomdns:
    image: ghcr.io/DrAlanK/phantomdns:latest
    restart: unless-stopped
    environment:
...
```bash
/container mounts
add dst=/data list=phantomdns src=/containers/mounts/phantomdns

/container envs
add key=DOMAIN list=phantomdns value=v.example.com

/container add check-certificate=no dns=1.1.1.1 envlists=phantomdns hostname=phantomdns interface=phantomdns layer-dir="" mountlists=phantomdns name=phantomdns remote-image=ghcr.io/DrAlanK/phantomdns:latest root-dir=/containers/data/phantomdns start-on-boot=yes
```
...
```bash
unzip phantomdns_Client_Linux_AMD64.zip
ls
```
...
```bash
chmod +x phantomdns_Client_Linux_AMD64
chmod +x phantomdns_Server_Linux_AMD64
```
...
```bash
./phantomdns_Client_Linux_AMD64
./phantomdns_Server_Linux_AMD64
```
...
go build -o phantomdns-client ./cmd/client
go build -o phantomdns-server ./cmd/server
```
...
go build -o phantomdns-client.exe .\cmd\client
go build -o phantomdns-server.exe .\cmd\server
```
...
```bash
./phantomdns-server -config server_config.toml
./phantomdns-client -config client_config.toml
```

На Windows:

```powershell
.\phantomdns-server.exe -config server_config.toml
.\phantomdns-client.exe -config client_config.toml
```
...
```bash
./phantomdns-server -config server_config.toml -log server.log
./phantomdns-client -config client_config.toml -log client.log
```
...
| `MTU_SERVERS_FILE_NAME` | `"phantomdns_success_test_{time}.log"` | Строка | Шаблон имени выходного файла для резолверов, успешно прошедших MTU-тестирование. |
...
Если на сервере уже установлена другая панель управления или запущена другая служба, вы можете настроить их исходящий трафик на локальный SOCKS5-сервер клиента phantomdns и использовать его в качестве выходного маршрута.
...
### Раздел 5.1: Проекты сообщества, связанные с phantomdns 🧩

В этом разделе мы представляем несколько проектов, связанных с phantomdns, которые были разработаны другими людьми. Они могут пригодиться для практического применения, в качестве источника вдохновения или для экспериментов. Обратите внимание, что эти проекты не связаны с самим phantomdns и могут работать иначе или включать собственные дополнительные функции и особенности дизайна.
...
- [MDV HN Edition Android Client](https://github.com/Hidden-Node/phantomdns-AndroidClient)
  Приложение для Android, созданное на базе phantomdns, которое позволяет подключаться к серверам phantomdns с телефона. Этот проект разработан Hidden Node.

- [phantomdns GG Android Client](https://github.com/RevocGG/phantomdns-AndroidGG)
  Еще один Android-клиент, созданный на базе phantomdns. Этот проект разрабатывается RevocGG.
- [Persian Config Builder for phantomdns](https://github.com/datacoder-io/phantomdns-ConfigMaker) - [Web Config Builder](https://datacoder-io.github.io/phantomdns-ConfigMaker)
  Инструмент, помогающий создавать конфигурационные файлы phantomdns с пояснениями на персидском языке и интуитивно понятным интерфейсом. Этот проект разработан datacoder-io

- [MasterDnsWeb (Web Management Client)](https://github.com/abolix/MasterDnsWeb)
  Веб-клиент для управления phantomdns, предназначенный для удобного управления одним или несколькими экземплярами одновременно. Разработан Abolix.

  - [KevinNet DNS](https://github.com/kamalalhagh/kevinnet-dns)
  Вспомогательный инструмент для phantomdns, который находит и проверяет работоспособность DNS-резолверов, в первую очередь для пользователей из Ирана. Он генерирует готовые к использованию файлы конфигурации и выполняет сквозную проверку туннеля. Этот проект разработан Kevin Haji.
...
Вы можете использовать клиент phantomdns в качестве локального прокси-сервера в таких панелях управления, как 3X-UI, или в любой другой системе, поддерживающей локальные исходящие прокси-серверы.
Выполните следующие действия:

- Запустите клиент phantomdns на том же сервере, где работает ваша панель управления.
...
- Выберите тег (например: `phantomdns-Out`).
- В поле **Address** введите локальный IP-адрес клиента phantomdns (обычно `127.0.0.1`).
- В поле **Порт** введите `LISTEN_PORT`, который вы задали в `client_config.toml` (например, `18000`).
- Если в phantomdns включена аутентификация SOCKS, введите здесь то же имя пользователя и пароль.
...
- В поле **Исходящие теги** выберите тег `phantomdns-Out`.
- Нажмите **Сохранить** и **Перезапустить Xray**.

Теперь весь трафик для этого входящего соединения будет проходить через phantomdns и выходить через ваш DNS-туннель.

---

## Раздел 6: 🛠️ Архитектура системы и принципы её работы

**phantomdns** сочетает в себе мультиплексирование сеансов, балансировку резолверов, дублирование пакетов, настраиваемый уровень ARQ и отложенное выполнение сеансов на основе протоколов UDP/DNS, что позволяет обойти типичные ограничения обычных туннелей.
...
[Issues](https://github.com/DrAlanK/phantomdns/issues)

[Pull Requests](https://github.com/DrAlanK/phantomdns/pulls)
...
Developed with ❤️ by: [**DrAlanK**](https://github.com/DrAlanK)
