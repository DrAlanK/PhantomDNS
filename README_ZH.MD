...
**phantomdns** 是一个面向科研的项目，旨在通过 DNS 查询和响应承载 TCP 流量。从总体目标上看，它与 DNSTT 或 SlipStream 等项目类似，但它遵循了根本不同的结构和实现思路。
...
| 特性 | SlipStream | DNSTT | phantomdns |
| :--- | :--- | :--- | :--- |
...
如需获取最新动态、发布信息和项目更新，请关注我们的 Telegram 频道：[Telegram Channel](https://t.me/phantomdns)
...
phantomdns 不只是一个理论项目。它经过实战检验，在全球互联网被完全切断的环境中也被证明可以工作。

最近，在伊朗持续 88 天的断网期间，当局不仅仅是封锁 VPN 或过滤网站——他们直接切断了国际带宽。在与外部世界的连接被物理切断 99% 的情况下，用户被困在一个封闭的本地内网之中。

当根本没有国际互联网可供连接时，标准的翻墙工具毫无用处。然而，在这场大规模断网期间，phantomdns 脱颖而出，成为极少数真正让用户保持连接到全球网络的“生命线”之一。

**它是如何在彻底断网中存活下来的？**
phantomdns 不像标准 VPN 那样工作，而是依靠智能的 DNS 隧道技术来穿透断网封锁：
...
正是这种精确的组合，使得 phantomdns 在外部世界被完全封锁时仍能维持稳定的连接。
...
| 操作系统 | 架构 | 适用于 | 直接下载 |
| :--- | :--- | :--- | :--- |
| Windows 🪟 | `AMD64`（64 位） | Windows 10 和 11 | [下载 Windows 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Windows_AMD64.zip) |
| Windows 🪟 | `x86`（32 位） | 较旧的 32 位 Windows 系统 | [下载 Windows x86 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Windows_X86.zip) |
| Windows 🪟 | `ARM64` | ARM 架构的 Windows 设备 | [下载 Windows ARM64 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Windows_ARM64.zip) |
| macOS 🍎 | `ARM64` | Apple Silicon Mac（M1 / M2 / M3） | [下载 macOS 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_MacOS_ARM64.zip) |
| macOS 🍎 | `AMD64` | Intel Mac | [下载 macOS Intel 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_MacOS_AMD64.zip) |
| Linux 🐧 | `AMD64`（64 位） | 现代发行版（Ubuntu 22.04+、Debian 12+） | [下载 Linux 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_AMD64.zip) |
| Linux 🐧 | `x86`（32 位） | 较旧的 32 位 Linux 系统 | [下载 Linux x86 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_X86.zip) |
| Linux（旧版）🐧 | `AMD64`（64 位） | 较旧的发行版（Ubuntu 20.04、Debian 11） | [下载 Linux 旧版客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux-Legacy_AMD64.zip) |
| Linux（旧版）🐧 | `ARM64` | 需要更广泛兼容性的较旧 ARM64 Linux 系统 | [下载 Linux 旧版 ARM64 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux-Legacy_ARM64.zip) |
| Linux（ARM）🐧 | `ARM64` | ARM 服务器、Raspberry Pi 及类似开发板 | [下载 Linux ARM 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARM64.zip) |
| Linux（ARM）🐧 | `ARMv7` | 32 位 ARM 开发板及较旧的嵌入式 Linux 设备 | [下载 Linux ARMv7 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARMV7.zip) |
| Linux（ARM）🐧 | `ARMv6` | 较旧的 ARM 开发板及轻量级 Linux 设备 | [下载 Linux ARMv6 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARMV6.zip) |
| Linux（ARM）🐧 | `ARMv5` | 非常老旧的 ARM 设备及嵌入式 Linux 系统 | [下载 Linux ARMv5 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_ARMV5.zip) |
| Linux 🐧 | `RISCV64` | RISC-V Linux 开发板及服务器 | [下载 Linux RISCV64 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_RISCV64.zip) |
| Linux（MIPS）🐧 | `MIPS` | 大端 MIPS Linux 及路由器平台 | [下载 Linux MIPS 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPS.zip) |
| Linux（MIPS）🐧 | `MIPSLE` | 小端 MIPS Linux 及路由器平台 | [下载 Linux MIPSLE 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPSLE.zip) |
| Linux（MIPS）🐧 | `MIPS64` | 64 位大端 MIPS Linux 系统 | [下载 Linux MIPS64 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPS64.zip) |
| Linux（MIPS）🐧 | `MIPS64LE` | 64 位小端 MIPS Linux 系统 | [下载 Linux MIPS64LE 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Linux_MIPS64LE.zip) |
| Termux / Android 📱 | `ARM64` | 运行 Termux 的现代 Android 手机 | [下载 Termux ARM64 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Termux_ARM64.zip) |
| Termux / Android 📱 | `ARMv7` | 运行 32 位 Termux 环境的较旧 Android 手机 | [下载 Termux ARMv7 客户端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Client_Termux_ARMV7.zip) |
...
| 操作系统 | 架构 | 适用于 | 直接下载 |
| :--- | :--- | :--- | :--- |
| Windows 🪟 | `AMD64`（64 位） | Windows Server、Windows 10 和 11 | [下载 Windows 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Windows_AMD64.zip) |
| Windows 🪟 | `x86`（32 位） | 较旧的 32 位 Windows 系统 | [下载 Windows x86 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Windows_X86.zip) |
| Windows 🪟 | `ARM64` | ARM 架构的 Windows 设备 | [下载 Windows ARM64 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Windows_ARM64.zip) |
| Linux 🐧 | `AMD64`（64 位） | Ubuntu 22.04+、Debian 12+ 服务器 | [下载 Linux 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_AMD64.zip) |
| Linux 🐧 | `x86`（32 位） | 较旧的 32 位 Linux 系统 | [下载 Linux x86 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_X86.zip) |
| Linux（旧版）🐧 | `AMD64`（64 位） | 较旧的服务器（Ubuntu 20.04、Debian 11） | [下载 Linux 旧版服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux-Legacy_AMD64.zip) |
| Linux（旧版）🐧 | `ARM64` | 需要更广泛兼容性的较旧 ARM64 Linux 系统 | [下载 Linux 旧版 ARM64 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux-Legacy_ARM64.zip) |
| Linux（ARM）🐧 | `ARM64` | ARM 服务器 | [下载 Linux ARM 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARM64.zip) |
| Linux（ARM）🐧 | `ARMv7` | 32 位 ARM 服务器及嵌入式 Linux 设备 | [下载 Linux ARMv7 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARMV7.zip) |
| Linux（ARM）🐧 | `ARMv6` | 较旧的 ARM 开发板及轻量级 Linux 设备 | [下载 Linux ARMv6 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARMV6.zip) |
| Linux（ARM）🐧 | `ARMv5` | 非常老旧的 ARM 设备及嵌入式 Linux 系统 | [下载 Linux ARMv5 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_ARMV5.zip) |
| Linux 🐧 | `RISCV64` | RISC-V Linux 开发板及服务器 | [下载 Linux RISCV64 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_RISCV64.zip) |
| Linux（MIPS）🐧 | `MIPS` | 大端 MIPS Linux 及路由器平台 | [下载 Linux MIPS 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPS.zip) |
| Linux（MIPS）🐧 | `MIPSLE` | 小端 MIPS Linux 及路由器平台 | [下载 Linux MIPSLE 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPSLE.zip) |
| Linux（MIPS）🐧 | `MIPS64` | 64 位大端 MIPS Linux 系统 | [下载 Linux MIPS64 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPS64.zip) |
| Linux（MIPS）🐧 | `MIPS64LE` | 64 位小端 MIPS Linux 系统 | [下载 Linux MIPS64LE 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Linux_MIPS64LE.zip) |
| macOS 🍎 | `ARM64` | Apple Silicon Mac | [下载 macOS 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_MacOS_ARM64.zip) |
| macOS 🍎 | `AMD64` | Intel Mac | [下载 macOS Intel 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_MacOS_AMD64.zip) |
| Termux / Android 📱 | `ARM64` | 现代 Android / Termux 环境 | [下载 Termux ARM64 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Termux_ARM64.zip) |
| Termux / Android 📱 | `ARMv7` | 较旧的 Android / 32 位 Termux 环境 | [下载 Termux ARMv7 服务端 ⬇️](https://github.com/DrAlanK/phantomdns/releases/latest/download/phantomdns_Server_Termux_ARMV7.zip) |

---

### 第 2.2 节：📦 phantomdns Docker 镜像

---

#### 第 2.2.1 节：⚠️ 概述

该 Docker 镜像在容器化环境中运行 phantomdns 服务端，并支持多架构构建。
...
| `MTU_SERVERS_FILE_NAME` | `"phantomdns_success_test_{time}.log"` | 字符串 | MTU 测试成功的解析器的输出文件名模板。 |
...
如果你已经在某台服务器上有了其他面板或服务，你可以将其出站路径指向 phantomdns 客户端的本地 SOCKS5，并将其用作出口路径。
...
### 第 5.1 节：与 phantomdns 相关的社区项目 🧩

在本节中，我们介绍几个由其他人开发的、与 phantomdns 相关的项目。它们可能对实际使用、获取灵感或进行试验有所帮助。请注意，这些项目独立于 phantomdns 本身，可能表现不同，或包含它们自己额外的特性和设计选择。
...
- [MDV HN Edition Android Client](https://github.com/Hidden-Node/phantomdns-AndroidClient)
  一个围绕 phantomdns 构建的 Android 应用，允许从手机连接到 phantomdns 服务器。该项目由 Hidden Node 独立开发。

- [phantomdns GG Android Client](https://github.com/RevocGG/phantomdns-AndroidGG)
  另一个围绕 phantomdns 构建的 Android 客户端。该项目由 RevocGG 独立开发。
...
- [Persian Config Builder for phantomdns](https://github.com/datacoder-io/phantomdns-ConfigMaker) - [Web Config Builder](https://datacoder-io.github.io/phantomdns-ConfigMaker)
  一个借助波斯语说明和引导式界面帮助创建 phantomdns 配置文件的工具。该项目由 datacoder-io 独立开发。

- [MasterDnsWeb (Web Management Client)](https://github.com/abolix/MasterDnsWeb)
  一个面向 phantomdns 的基于 Web 的管理客户端，旨在让同时管理一个或多个实例变得简单。由 Abolix 独立开发。

- [KevinNet DNS](https://github.com/kamalalhagh/kevinnet-dns)
  一个面向 phantomdns 的配套工具，可发现并验证可用的 DNS 解析器，尤其针对伊朗的用户。它会生成开箱即用的配置文件并执行端到端的隧道验证。该项目由 Kevin Haji 独立开发。
...
你可以将 phantomdns 客户端用作 3X-UI 等面板或任何支持本地出站代理系统内部的本地代理。
请按以下步骤操作：

- 在运行你面板的同一台服务器上运行 phantomdns 客户端。
- 在 3X-UI 中，进入 **Inbound** 并创建一个新的入站（VLESS/VMess 或你想要的任何协议）。
- 打开 **Xray Configs**，然后进入 **Outbound** 选项卡。
- 点击 **Add Outbound** 创建一个新的出站。
- 将 **Protocol** 设为 **Socks**。
- 选择一个标签（例如：`phantomdns-Out`）。
- 在 **Address** 中，输入 phantomdns 客户端的本地 IP（通常是 `127.0.0.1`）。
- 在 **Port** 中，输入你在 `client_config.toml` 中设置的 `LISTEN_PORT`（例如 `18000`）。
- 如果 phantomdns 中启用了 SOCKS 认证，请在此输入相同的用户名/密码。
- 点击 **Add Outbound** 进行保存。
- 进入 **Routing Rules** 选项卡并点击 **Add Rule**。
- 在 **Inbound Tags** 中，选择你创建的入站。
- 在 **Outbound Tags** 中，选择 `phantomdns-Out` 标签。
- 点击 **Save** 和 **Restart Xray**。

现在，该入站的所有流量都将经过 phantomdns，并通过你的 DNS 隧道出口。

---

## 第 6 部分：🛠️ 系统架构与工作原理

**phantomdns** 在 UDP/DNS 之上结合了会话复用（Session Multiplexing）、解析器均衡（Resolver Balancing）、数据包冗余复制（Packet Duplication）、一个自定义 ARQ 层以及一个延迟会话运行时（Deferred Session Runtime），以避免普通隧道常见的局限性。
...
由 [**DrAlanK**](https://github.com/DrAlanK) 用 ❤️ 开发
