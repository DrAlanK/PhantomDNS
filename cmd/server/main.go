...
import (
	...
	"phantomdns-go/internal/runtimepath"
	"phantomdns-go/internal/security"
	UDPServer "phantomdns-go/internal/udpserver"
	"phantomdns-go/internal/version"
)
...
func main() {
	...
	if *versionFlag {
		fmt.Printf("phantomdns Server Version: %s\n", version.GetVersion())
		return
	}
	...
	if *logPath != "" {
		log = logger.NewWithFile("phantomdns Server", cfg.LogLevel, *logPath)
	} else {
		log = logger.New("phantomdns Server", cfg.LogLevel)
	}

	log.Infof("============================================================")
	log.Infof("<cyan>GitHub:</cyan> <yellow>https://github.com/DrAlanK/phantomdns</yellow>")
	log.Infof("<cyan>Telegram:</cyan> <yellow>@phantomdns</yellow>")
	log.Infof("<cyan>Build Version:</cyan> <yellow>%s</yellow>", version.GetVersion())
	log.Infof("============================================================")

	log.Infof("\U0001F680 <magenta>phantomdns Server starting ...</magenta>")
